...
const ChipsArray = () => {
  ...
  const onAutocompleteChange = (e, value) => {
    ...
    if(value!==null){
      ...
      dispatch(setChosenTags(chosenTags.filter(item => item!==selectedTag)))
    }
  }
  ...
  return (
    <div className={classes.container}>
        ...
        />
      {chosenTags.map(
        chosenTag => <Chip key={chosenTag.id} label={chosenTag.tag} onDelete={()=>onTagClose(chosenTag)} className={classes.chip} classes={{deleteIcon: classes.deleteIconColor}}/>
      )}
    </div>
  );
}
