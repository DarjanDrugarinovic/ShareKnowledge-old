...
const Heading = (props) => {
  ...
  return (
    <List className={classes.list}>
      <ListItem button onClick={() => { if(!hideLeftBar) setOpen(!open); }} className={classes.arrowColor}>
        {open && !hideLeftBar ? <ExpandLess /> : <ExpandMore />}
        <ListItemText 
        disableTypography
        primary={<Typography type="body2" className={classes.textColor}>{name} </Typography>}
        />
